...
export default function LiteratureForm() {  
  ...
  const saveBook = async () => {
    //A NEW ITEM
    let new_data = { "id": key, "name": name, "detail": detail,"price": price, "image": image, "catagory": "literature" };
    //SAVE
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, padding: 20 }}>
      <ScrollView>
        ...
        <Text>หมวดหมู่</Text>
        <TextInput placeholder="Enter name ..." value={literature} onChangeText={(text) => setLiterature(text)} />
        <Text>ราคา</Text>
